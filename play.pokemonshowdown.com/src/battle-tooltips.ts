...
class BattleTooltips {
	...
	calculateModifiedStats(clientPokemon: Pokemon | null, serverPokemon: ServerPokemon, statStagesOnly?: boolean) {
		...
		if (weather) {
			if (!this.battle.tier.includes("Roovnen")) {
				...
				if (item !== 'utilityumbrella') {
					if (this.pokemonHasType(pokemon, 'Rock') && weather === 'sandstorm') {
						stats.spd = Math.floor(stats.spd * 1.5);
					}
					if (this.pokemonHasType(pokemon, 'Ice') && weather === 'snow') {
						stats.def = Math.floor(stats.def * 1.5);
					...
					if (ability === 'hailforce' && (weather === 'snow' || weather === 'heavyblizzards')) {
						stats.atk = Math.floor(stats.atk * 1.5);
					}
				}
			}
		...
		if (clientPokemon) {
			...
			for (const statName of Dex.statNamesExceptHP) {
				...
				if (clientPokemon.volatiles['protosynthesis' + statName] && this.battle.tier.includes("Roovnen")) {
					if (statName === 'spe') {
						speedModifiers.push(1.5);
					} else {
		...
		if (ability === 'maternalguard' && isNFE && this.battle.tier.includes("Roovnen")) {
			stats.def = Math.floor(stats.def * 1.5);
			stats.spd = Math.floor(stats.spd * 1.5);
		}
	...
	getMoveAccuracy(move: Move, value: ModifiableValue, target?: Pokemon) {
		...
		if (move.id === 'blizzard' && this.battle.tier.includes("Roovnen")) {
	...
	getMoveBasePower(move: Move, moveType: TypeName, value: ModifiableValue, target: Pokemon | null = null) {
		...
		if (move.id === 'lastrespects') {
			let lrNR = this.battle.tier.includes("Roovnen") ? 25 : 50;
			value.set(Math.min(50 + lrNR * pokemon.side.faintCounter));
		}
		...
		if (['Rock', 'Ground', 'Steel'].includes(moveType) && this.battle.weather === 'sandstorm' && !this.battle.tier.includes("Roovnen")) {
