...
class BattleTooltips {
			...
			if (item.zMoveFrom === move.name) {
				...
				if (move.id === 'weatherball') {
					...
					case 'primordialsea':
						...
						break;
					case 'sandstorm':
						zMove = this.battle.tier.includes("TCG") ? this.battle.dex.moves.get(BattleTooltips.zMoveTable['Fighting']) : this.battle.dex.moves.get(BattleTooltips.zMoveTable['Rock']);
						break;
					case 'hail':
					case 'snow':
						zMove = this.battle.tier.includes("TCG") ? this.battle.dex.moves.get(BattleTooltips.zMoveTable['Water']) : this.battle.dex.moves.get(BattleTooltips.zMoveTable['Ice']);
						break;
					}
				}
	...
	calculateModifiedStats(clientPokemon: Pokemon | null, serverPokemon: ServerPokemon, statStagesOnly?: boolean) {
		...
		if (weather) {
			if (this.battle.gen >= 4 && this.pokemonHasType(pokemon, 'Fighting') && weather === 'sandstorm' && this.battle.tier.includes("TCG")) {
				stats.spd = Math.floor(stats.spd * 1.5);
			}
			if (this.battle.gen >= 4 && this.pokemonHasType(pokemon, 'Rock') && weather === 'sandstorm') {
				stats.spd = Math.floor(stats.spd * 1.5);
			}
			if (this.pokemonHasType(pokemon, 'Water') && weather === 'snow' && this.battle.tier.includes("TCG")) {
				stats.def = Math.floor(stats.def * 1.5);
			}
	...
	getMoveType(move: Move, value: ModifiableValue, forMaxMove?: boolean | Move): [TypeName, 'Physical' | 'Special' | 'Status'] {
		...
		//Type changes
		if (this.battle.tier.includes("TCG")) {
			if (['attackorder', 'bugbite', 'bugbuzz', 'defendorder', 'fellstinger', 'firstimpression', 'furycutter', 'infestation', 'leechlife', 'lunge', 'megahorn', 'pinmissile', 'pollenpuff', 'pounce', 'quiverdance', 'ragepowder', 
				'silktrap', 'skittersmack', 'stickyweb', 'stringshot', 'strugglebug', 'tailglow', 'uturn', 'xscissor', 'healorder', 'powder', 'signalbeam', 'silverwind', 'spiderweb', 'steamroller', 'twineedle'].includes(move.id)) {
				moveType = 'Grass';
			} else if (['alluringvoice', 'aromaticmist', 'babydolleyes', 'charm', 'dazzlinggleam', 'decorate', 'disarmingvoice', 'drainingkiss', 'fairylock', 'fairywind', 'fleurcannon', 'floralhealing', 'magicaltorque', 'mistyexplosion', 
				'mistyterrain', 'moonblast', 'moonlight', 'playrough', 'spiritbreak', 'springtidestorm', 'strangesteam', 'sweetkiss', 'craftyshield', 'flowershield', 'geomancy', 'guardianofalola', 'letssnuggleforever', 'lightofruin', 
				'naturesmadness', 'sparklyswirl', 'twinkletackle', 'astonish', 'astralbarrage', 'bittermalice', 'confuseray', 'curse', 'destinybond', 'hex', 'infernalparade', 'lastrespects', 'lick', 'moongeistbeam', 'nightshade', 
				'phantomforce', 'poltergeist', 'ragefist', 'shadowball', 'shadowclaw', 'shadowforce', 'shadowpunch', 'shadowsneak', 'spiritshackle', 'spite', 'grudge', 'menacingmoonrazemaelstrom', 'neverendingnightmare', 'nightmare', 
				'ominouswind', 'shadowbone', 'shadowstrike', 'sinisterarrowraid', 'soulstealing7starstrike', 'spectralthief', 'trickortreat'].includes(move.id)) {
				moveType = 'Psychic';
			} else if (['acrobatics', 'aerialace', 'aeroblast', 'aircutter', 'airslash', 'beakblast', 'bleakwindstorm', 'bounce', 'bravebird', 'defog', 'dragonascent', 'drillpeck', 'dualwingbeat', 'featherdance', 'fly', 'gust', 'hurricane', 
				'peck', 'pluck', 'roost', 'skyattack', 'tailwind', 'wingattack', 'chatter', 'floatyfall', 'mirrormove', 'oblivionwing', 'skydrop', 'supersonicskystrike'].includes(move.id)) {
				moveType = 'Normal';
			} else if (['', 'bonerush', 'bulldoze', 'dig', 'drillrun', 'earthpower', 'earthquake', 'fissure', 'headlongrush', 'highhorsepower', 'mudshot', 'mudslap', 'precipiceblades', 'sandattack', 'sandtomb', 'sandsearstorm', 'scorchingsands', 
				'shoreup', 'spikes', 'stompingtantrum', 'boneclub', 'bonemerang', 'landswrath', 'magnitude', 'mudbomb', 'mudsport', 'rototiller', 'tectonicrage', 'thousandarrows', 'thousandwaves', 'accelerock', 'ancientpower', 'diamondstorm', 
				'headsmash', 'meteorbeam', 'mightycleave', 'powergem', 'rockblast', 'rockpolish', 'rockslide', 'rockthrow', 'rocktomb', 'rockwrecker', 'rollout', 'saltcure', 'sandstorm', 'smackdown', 'stealthrock', 'stoneaxe', 'stoneedge', 
				'tarshot', 'wideguard', 'continentalcrush', 'paleowave', 'splinteredstormshards'].includes(move.id)) {
				moveType = 'Fighting';
			} else if (['aurorabeam', 'auroraveil', 'avalanche', 'blizzard', 'chillyreception', 'freezeshock', 'freezedry', 'frostbreath', 'glaciallance', 'glaciate', 'haze', 'icebeam', 'iceburn', 'icefang', 'icehammer', 'icepunch', 'iceshard', 
				'icespinner', 'iciclecrash', 'iciclespear', 'icywind', 'mist', 'mountaingale', 'powdersnow', 'sheercold', 'snowscape', 'tripleaxel', 'freezyfrost', 'hail', 'iceball', 'subzeroslammer'].includes(move.id)) {
				moveType = 'Water';
			} else if (['acid', 'acidarmor', 'acidspray', 'banefulbunker', 'barbbarrage', 'belch', 'clearsmog', 'coil', 'corrosivegas', 'crosspoison', 'direclaw', 'gastroacid', 'gunkshot', 'malignantchain', 'mortalspin', 'noxioustorque', 'poisonfang', 
				'poisongas', 'poisonjab', 'poisonpowder', 'poisonsting', 'poisontail', 'shellsidearm', 'sludge', 'sludgebomb', 'sludgewave', 'smog', 'toxic', 'toxicspikes', 'toxicthread', 'venoshock', 'aciddownpour', 'purify', 'venomdrench'].includes(move.id)) {
				moveType = 'Dark';
			}
		}
		...
		if (move.id === 'weatherball' && value.weatherModify(0)) {
			...
			case 'primordialsea':
				...
				break;
			case 'sandstorm':
				moveType = this.battle.tier.includes("TCG") ? 'Fighting' : 'Rock';
				break;
			case 'hail':
			case 'snow':
				moveType = this.battle.tier.includes("TCG") ? 'Water' : 'Ice';
				break;
			}
		}
		if (move.id === 'terrainpulse' && pokemon.isGrounded(serverPokemon)) {
			if (this.battle.hasPseudoWeather('Electric Terrain')) {
				...
				moveType = this.battle.tier.includes("TCG") ? 'Psychic' : 'Fairy';
			} else if (this.battle.hasPseudoWeather('Psychic Terrain')) {
				moveType = 'Psychic';
			}
		}
		...
		if (allowTypeOverride) {
			...
			if (category !== 'Status' && !move.isZ && !move.id.startsWith('hiddenpower')) {
				if (moveType === 'Normal') {
					if (value.abilityModify(0, 'Aerilate') && !this.battle.tier.includes("TCG")) moveType = 'Flying';
					if (value.abilityModify(0, 'Galvanize')) moveType = 'Electric';
					if (value.abilityModify(0, 'Pixilate')) moveType = this.battle.tier.includes("TCG") ? 'Psychic' : 'Fairy';
					if (value.abilityModify(0, 'Refrigerate')) moveType = this.battle.tier.includes("TCG") ? 'Water' : 'Ice';
				}
	...
	getMoveAccuracy(move: Move, value: ModifiableValue, target?: Pokemon) {
		...
		if (move.id === 'toxic' && this.battle.gen >= 6 && this.pokemonHasType(pokemon, 'Dark') && this.battle.tier.includes("TCG")) {
			value.set(0, "Dark type");
			return value;
		}
		...
		if (move.ohko) {
			...
			if (move.id === 'sheercold' && this.battle.gen >= 7 && !this.pokemonHasType(pokemon, 'Water') && this.battle.tier.includes("TCG")) {
				value.set(20, 'not Water-type');
			}
			if (target) {
	...
	getMoveBasePower(move: Move, moveType: TypeName, value: ModifiableValue, target: Pokemon | null = null) {
		...
		if (moveType === 'Fighting' && this.battle.weather === 'sandstorm' && this.battle.tier.includes("TCG")) {
			if (value.tryAbility("Sand Force")) value.weatherModify(1.3, "Sandstorm", "Sand Force");
		}
		if (move.secondaries) {
		...
		if (move.category !== 'Status') {
			...
			for (const ally of pokemon.side.active) {
				if (!ally || ally.fainted) continue;
				let allyAbility = this.getAllyAbility(ally);
				if (moveType === 'Fairy' && allyAbility === 'Fairy Aura' && !this.battle.tier.includes("TCG")) {
					auraBoosted = 'Fairy Aura';
				} else if (moveType === 'Psychic' && allyAbility === 'Fairy Aura' && this.battle.tier.includes("TCG")) {
					auraBoosted = 'Fairy Aura';
			...
			for (const foe of pokemon.side.foe.active) {
				...
				if (foe.ability === 'Fairy Aura' && moveType === 'Fairy' && !this.battle.tier.includes("TCG")) {
					auraBoosted = 'Fairy Aura';
				} else if (foe.ability === 'Fairy Aura' && moveType === 'Psychic' && this.battle.tier.includes("TCG")) {
					auraBoosted = 'Fairy Aura';
	...
	static orbTypes: {[itemName: string]: TypeName[]} = {
		...
		'Griseous Core': ['Psychic', 'Dragon'],
		'Griseous Orb': ['Psychic', 'Dragon'],
		'Vile Vial': ['Dark', 'Normal'],
	};
	static noGemMoves = [
		'Fire Pledge',
		'Fling',
...
class BattleStatGuesser {
	...
	guessEVs(set: PokemonSet, role: string): Partial<StatsTable> & {plusStat?: StatName | '', minusStat?: StatName | ''} {
		...
		if (this.supportsAVs) {
			...
			let SRweaknesses = ['Dark', 'Steel'];
			let SRresistances = ['Psychic'];
			let SRweak = 0;
